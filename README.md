# Open_Topography_Data_Download _API
...
A lightweight, interactive Python notebook for downloading global elevation (DEM) data from the OpenTopography Global DEM API — no manual portal navigation, no GIS software, just bounding-box coordinates and an API key.

Overview

This notebook is a simple, guided 4-step interactive workflow:

Enter bounds and API key — North/South/West/East bounds in decimal degrees, plus a personal OpenTopography API key
Select a DEM product — choose from 13 supported global elevation/bathymetry datasets via a numbered menu
Send the download request — builds and sends the API request, with human-readable interpretation of HTTP response codes
Download the file — writes the returned raster to disk as a GeoTIFF
...
API key required: OpenTopography requires a free personal API key (register at portal.opentopography.org) — the notebook prompts for it interactively rather than hardcoding it, keeping credentials out of the notebook file itself.
...
Register for a free API key at portal.opentopography.org
Open the notebook and run each cell in order, entering bounds/key/product/filename when prompted
